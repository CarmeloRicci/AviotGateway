...
export default class DnsService {
    ...
    async SendPostRequest (firstdns: string){
        ...
        let request_data = {
            url: `http://${cfg.general.ipDnsServer}:${cfg.general.portDnsServer}/dns/dns_request`,
            method: 'POST',
            body: {
                params: {
                    ipdns: firstdns
        ...
        await console.log("DnsService - SendPostRequest: Post send! " + `(http://${cfg.general.ipDnsServer}:${cfg.general.portDnsServer}/dns/dns_request)`)
    }
